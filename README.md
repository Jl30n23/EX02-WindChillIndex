...
from GitHub Classroom. You can click [here](https://classroom.github.com/a/RIf5VDc-) to accept the invitatiion. 
...
Write your declaration of the `computeWindChillIndex` in here in the `edu::sbcc::cs140` namepspace.
...
Write your implementation of the `computeWindChillIndex` in here in the `edu::sbcc::cs140` namepspace.
...
Make sure only the files you want to push are selected, `main.cc`, `windchillindex.h`, and `windchillindex.cc`. Then uncheck `Perform code analysis` and `Check TODO`. It's OK to leave them checked, but committing will take longer. Leave `Run git hooks`, if it's visible, unchecked as well. Put a message in `Commit Message` and then press the **Commit** button. If anything goes wrong check the _Version Control_ view
